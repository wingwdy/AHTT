...
Test-RequiredText -Name 'M2-SRC-001 login sender uses Version and ICCID' -Path $sendPath -Pattern '#include\s+"Version\.h"[\s\S]*IotAHTT_SendLoginReq[\s\S]*CddNetM_GetIccid[\s\S]*APP_SW_MINOR_VERSION[\s\S]*APP_SW_CUSTORM_VERSION\s*\*\s*10\s*\+\s*APP_SW_PATCH_VERSION[\s\S]*IOT_AHTT_METER_ADDR_LEN'
...
Test-RequiredText -Name 'M2-SRC-007 heartbeat sender table and parameters' -Path $sendPath -Pattern '\{IOT_AHTT_CMD_HEARTBEAT,\s*IOT_AHTT_CMDTYPE_REQUSET,\s*0,\s*IotAHTT_SendHeartBeat,\s*IOT_AHTT_CMD_HEARTBEAT[\s\S]*IotAHTT_SendHeartBeat[\s\S]*IOT_AHTT_HEART_NET_4G[\s\S]*CddNetM_GetCsq[\s\S]*memset\(&pBuf\[dataLen\],\s*0xFF,\s*IOT_AHTT_HEART_STATE_LEN\)[\s\S]*IOT_AHTT_HEART_CHANNEL_COUNT'
