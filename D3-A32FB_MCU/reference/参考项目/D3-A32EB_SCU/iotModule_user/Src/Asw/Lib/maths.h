...
#define ARRAYBYTESWAP(x) (x[1] | (x[0] << 8))
#define ARRAYLONGSWAP(x) ((x[0] << 24) | \
							(x[1] << 16) | \
							(x[2] << 8)  | \
							(x[3]))
#define ARRAYTOSHORT(x)  (x[0] | (x[1] << 8))
#define ARRAYTOINT(x) ((x[3] << 24) | \
									(x[2] << 16) | \
									(x[1] << 8)  | \
									(x[0]))
...
#define HALF_CARRY(a)  ((a%2)?(a/2+1):(a/2))
...
#define out_range(data,min,max) ((data < min) || (data > max))
