...
完整的 Planner、Executor、Reviewer 角色定义和 Web → Desktop Handoff 位于：

```text
docs/ai/AHTT_AI_Development_Prompts.md
```

当用户明确要求进入 Planner、Executor 或 Reviewer 阶段时，必须先读取该文件中对应角色的规则，并且一次只执行一个角色。

Web / Cloud Planner 可能无法访问本机 Global `AGENTS.md` 和本机 Global Skills；这不构成阻断。此时至少必须读取当前仓库 `AGENTS.md`、当前任务所需的项目级 Skills、当前代码和 `docs/ahtt/` 权威文档。Desktop 环境可在项目规则基础上继续叠加本机 Global 规则，但不得覆盖本项目角色 Gate。
