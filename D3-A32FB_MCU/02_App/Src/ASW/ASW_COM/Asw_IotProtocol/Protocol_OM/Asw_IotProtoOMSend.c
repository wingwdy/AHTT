...
static uint16_t IotOM_GetPlatNo(void)
{
    ...
    switch (platType)
    {
        case eAswPlatType_GN:         platNo = 1;   break;
        case eAswPlatType_GNP:        platNo = 2;   break;
        case eAswPlatType_YKC16:      platNo = 3;   break;
        case eAswPlatType_XJ:         platNo = 6;   break;
        case eAswPlatType_TT24:       platNo = 9;   break;
     /* case eAswPlatType_GWE:        国网e充电为专网卡，不连运维平台，所以不定义类型 */
        case eAswPlatType_AP:         platNo = 12;  break;
        case eAswPlatType_XDT:        platNo = 14;  break;
        case eAswPlatType_YKC21:      platNo = 22;  break;
        case eAswPlatType_DXL:        platNo = 26;  break;
        case eAswPlatType_AHTT:       platNo = 29;  break;
        default:                      platNo = 0;   break;
    }
...
static uint16_t IotOM_SendMeterVal(uint8_t port, uint8_t *pBuf)
{
    ...
    pBuf[dataLen++] = port + 1;
    /* 有功功率 */
    Common_Uint32ToFourUint8(&pBuf[dataLen], AswChargeIf_GetOutputPower(port) * 10000);
    dataLen += 4;
    /* 电表底数 */
            ...
            if (pIotOMCtx->sendIndex < ARRAY_SIZE(c_stIotOMSendctrlTable))
            {
                    ...
                    (IotOM_ReportCycleCheck(port, pCmdSendCtrl->cmd, pCmdSendCtrl->sendCycle) == TRUE))
                {
                    ...
                    if (dataLen > 0)
                    {
                        ...
                        if (eGlobalRet_OK != FrameQueue_PushTx(pIotOMCtx->frameQueueChannelID, NULL, 0, txBuf, dataLen, 0))
                        {
                            if (pCmdSendCtrl->cmdType == IOT_OM_CMDTYPE_REQUSET)
                            {
                                pIotOMCtx->reqSeq--;
