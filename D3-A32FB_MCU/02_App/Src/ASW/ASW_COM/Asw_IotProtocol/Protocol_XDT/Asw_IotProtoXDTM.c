...
uint8_t IotXDT_SwipCardCharge(uint8_t port, uint8_t *pCardID)
{
    uint8_t ret = FALSE;
    ...
    if (pIotXDTCtx->loginSucc == TRUE)
    {
        if (port < SYSCFG_CFG_GUN_NUM)
        {
            ...
            {
                IOTXDT_CFG_InfoPrint("[枪：%d]刷卡成功，但是已经有卡在申请启动充电，本次刷卡作废!\r\n", port);
            }
        }
    }
    else
    {
        IOTXDT_CFG_InfoPrint("[枪：%d]刷卡成功，设备离线，拒绝充电!!\r\n", port);
    }
...
uint8_t IotXDT_SetProductKey(char *pProductKey, uint8_t len)
{
    ...
    if (len <= MSNVM_XDT_PRODUCT_KEY_LEN)
    {
        ...
        ret = (MSNvm_WriteParaBlock(eMSNvmBlockID_PlatPrivateParam, (uint8_t *)pPrivateParam, sizeof(MSNvmPlatPrivateParam_Union)) == eGlobalRet_OK) ? TRUE : FALSE;
    }
...
uint8_t IotXDT_FillLinkPara(CddNetMSocketPara_Union *pLinkPara)
{
    MSNvmPlatParam_Struct *pParam = AswPlatM_GetPlatParamPtr();
    MSNvmPlatPrivateParam_Union *pPrivateParam = AswPlatM_GetPlatPrivateParamPtr();
