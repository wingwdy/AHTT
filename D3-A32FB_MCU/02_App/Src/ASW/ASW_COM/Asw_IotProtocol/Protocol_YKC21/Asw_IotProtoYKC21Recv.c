        ...
        if (pCmdRecvCtrl != NULL)
        {
            if (pCmdRecvCtrl->pRecvParse != NULL)
            {
                ...
                if (pCmdRecvCtrl->encryptionFlag)
                {
                    ...
                    IOTYKC21_CFG_DebugPrint("[枪：%d]接收密文消息体[cmd: 0x%02X, %s][%d]: ", port, pCmdRecvCtrl->cmd, 
                ...
                if (TRUE == pCmdRecvCtrl->pRecvParse(&port, IotYKC21Decryptbuf, decrypted_len))
                {
                    ...
                    {
                        IOTYKC21_CFG_DebugPrint("[枪：%d]接收明文消息体[cmd: 0x%02X, %s][%d]: ", port, pCmdRecvCtrl->cmd, pCmdRecvCtrl->cMeaning, decrypted_len);
...
static uint8_t IotYKC21_RecvUpdateAccountMoney(uint8_t *port, uint8_t *r_data, uint16_t len)
{
    ...
    else
    {
        index += 8;
        pChargeCtrl->accountMoney = Common_FourUint8ToUint32(&pRecvData[index]);
        IOTYKC21_CFG_InfoPrint("[枪：%d]更新账户余额成功，余额：%d.%02d!\r\n", port[0], pChargeCtrl->accountMoney / 100, 
...
void IotYKC21_TimeoutDetect(void)
{
    ...
    for (index = 0; index < IOT_YKC21_CMD_RECV_COUNT; index++)
    {
        ...
        for (port = 0; port < SYSCFG_CFG_GUN_NUM; port++)
        {
            ...
            if (Common_JudgeTimeoutMs(Common_GetRecvTick(pIotYKC21Ctx->pFuncRecvCtrl, port, pCmdRecvCtrl->cmd),pCmdRecvCtrl->maxTimeout) == TRUE)
            {
