...
static void shellParserParam(Shell *shell)
{
    ...
    for (i = 0; i < shell->parser.length; i++)
    {
        if (pariedCount == 0)
        {
            if (shell->parser.buffer[i] != ' ' && record == 1 && count < SHELL_PARAMETER_MAX_NUMBER)
            {
                shell->parser.param[count++] = &(shell->parser.buffer[i]);
                record = 0;
            }
            else if (shell->parser.buffer[i] == ' ' && record == 0)
            {
                shell->parser.buffer[i] = 0;
                if (shell->parser.buffer[i + 1] != ' ')
                {
                    record = 1;
                }
                continue;
        ...
        for (j = 0; j < sizeof(pairedChars) / 2; j++)
        {
            if (pariedCount > 0
