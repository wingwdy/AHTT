...
static uint16_t CddDrvEG800AK_TransparentRecvHandle(uint8_t *recvbuf, uint16_t dataLen);
static uint16_t CddDrvEG800AK_ATAnswerHandle(uint8_t *recvbuf, uint16_t dataLen);
...
static uint8_t CddDrvEG800AK_PowerOn(void)
{
    ...
    switch (g_stCddDrvEG800AKCtrl.powerOnStep)
    {
        case CDDDRV_EG800AK_CTRL_STEP0:
        {
            ...
            CDDDRV_EG800AK_CFG_InfoPrint("4G Module EG800AK PowerOff!\r\n");
            break;
        }
    ...
    while (findTimes < (CDDDRV_EG800AK_CFG_SOCKET_COUNT + 1))
    {
        ...
        if ((pAtCtrl != NULL) && (pAtCtrl->readyFlag == TRUE) && (pAtCtrl->atTaskArray[0] != 0))
        {
            ...
            if (pCmdDescribtor != NULL)
            {
                ...
                g_stCddDrvEG800AKCtrl.currentTaskCmdSendFlag = FALSE;
        ...
        switch (g_stCddDrvEG800AKCtrl.cmdTaskStep)
        {
            ...
            case CDDDRV_EG800AK_CTRL_STEP1:
            {
                ...
                if (txLen > 0)
                {
                    ...
                    g_stCddDrvEG800AKCtrl.currentTaskCmdSendFlag = TRUE;
                    txBuf[txLen] = 0;
                ...
                else
                {
                    ...
                    if (g_stCddDrvEG800AKCtrl.currentTaskCmd != 0)
                    {
                        /* 如果 g_stCddDrvEG800AKCtrl.currentTaskCmd == 0, 说明内部执行过删除，不需要重复删除 */
...
{
    CddDrvEG800AKSocketCtrl_Struct *pSocketCtrl = NULL;
    uint16_t transparentDealLen = dataLen;
    uint16_t remainLen = dataLen;
    uint8_t exitTransparent = TRUE;

    if (CddDrvEG800AK_CheckTransparentMode() == TRUE && g_stCddDrvEG800AKCtrl.eTransparentDirection == eCddDrvEG800AKDirection_Recv)
    {
        /* 默认整段由透传业务消费，只有回调明确返回较小长度时才处理尾部AT数据。 */
        ...
        {
            exitTransparent = c_stCddDrvEG800AKSocketConfigTable[pSocketCtrl->eSocketType].recvTransparentData(&g_stCddDrvEG800AKCtrl, recvbuf, dataLen, &transparentDealLen);
        }

        if (exitTransparent == TRUE)
        {
            CddDrvEG800AK_ExitTransparentMode();

            /* 升级数据收齐后，串口缓冲区中超出的部分属于随后到达的AT响应或URC。 */
            if (transparentDealLen <= dataLen)
            {
                remainLen = dataLen - transparentDealLen;
                if (remainLen > 0)
                {
                    CDDDRV_EG800AK_CFG_InfoPrint("FTP透明接收存在尾部AT数据：总长度=%d，升级数据=%d，剩余数据=%d\r\n", dataLen, transparentDealLen, remainLen);
                    /* 将尾部搬到缓冲区起始位置，交给原有AT响应和URC流程继续解析。 */
                    memmove(recvbuf, &recvbuf[transparentDealLen], remainLen);
                    recvbuf[remainLen] = 0;
                }
            }
            else
            {
                CDDDRV_EG800AK_CFG_DebugPrint("FTP透明接收处理长度异常：总长度=%d，处理长度=%d\r\n", dataLen, transparentDealLen);
                remainLen = 0;
            }
        }
        else
        {
            /* 升级数据尚未收齐，数据已存入FTP缓存，本轮不再进入AT解析。 */
            remainLen = 0;
        }
    }
...
static uint16_t CddDrvEG800AK_ATAnswerHandle(uint8_t *recvbuf, uint16_t dataLen)
{
    ...
    uint16_t dealLen = 0;
    uint16_t preLen = 0;
    uint16_t postLen = dataLen;
    uint16_t remainLen = dataLen;
    uint8_t bAnswerOK = FALSE;

    if (g_stCddDrvEG800AKCtrl.currentTaskCmd != 0 && g_stCddDrvEG800AKCtrl.currentTaskCmdSendFlag == TRUE)
    {
        ...
        if (pDest != NULL)
        {
            ...
            if (pATCmdDescribtor->pFuncRecvHandle != NULL)
            {
                if (socketIndex < CDDDRV_EG800AK_CFG_SOCKET_COUNT)
                {
                    pSocketCtrl = &g_stCddDrvEG800AKCtrl.stSocketCtrl[socketIndex];
            ...
            /* 未由接收回调给出处理长度时，根据AT响应头和响应尾计算已消费范围。 */
            if (dealLen == 0)
            {
            ...
            /* 防止回调返回的处理长度异常导致postLen下溢，进而引起后续内存越界。 */
            if (postLen > remainLen)
            {
                CDDDRV_EG800AK_CFG_DebugPrint("[4G] remainLen异常: postLen=%d, remainLen=%d\r\n", postLen, remainLen);
                postLen = 0;
            }
...
static void CddDrvEG800AK_ATTaskRecvHandle(uint8_t *recvbuf)
{
    ...
    if (dataLen > 0)
    {
        ...
        recvbuf[dataLen] = 0;
        remainLen = CddDrvEG800AK_TransparentRecvHandle(recvbuf, dataLen);

        if (remainLen > 0)
        {
            remainLen = CddDrvEG800AK_ATAnswerHandle(recvbuf, remainLen);
...
{ 
    if (Common_JudgeTimeoutMs(g_stCddDrvEG800AKCtrl.noCommTickStart ,CDDDRV_EG800AK_CFG_NO_COMM_TIMEOUT))
    {
        ...
        CDDDRV_EG800AK_CFG_InfoPrint("4G 模组无数据通信超时 %d ms\r\n!", CDDDRV_EG800AK_CFG_NO_COMM_TIMEOUT);    
    }
}
...
static uint8_t CddDrvEG800AK_FindFreeSocket(uint8_t *pSocketIndex)
{
	...
	for (socketIndex = 0; socketIndex < CDDDRV_EG800AK_CFG_SOCKET_COUNT; socketIndex++)
	{
		...
		if (pSocketCtrl->usedFlag == FALSE && pSocketCtrl->eSocketState == eCddNetMSocketState_Init)
		{
