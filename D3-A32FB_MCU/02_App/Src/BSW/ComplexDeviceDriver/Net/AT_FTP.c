...
typedef struct
{
    ...
    uint8_t *transCacheBuf;                     /* 透传缓存 */
    uint16_t transCacheLen;                     /* 已缓存大小 */
    uint16_t transExpectedLen;                  /* 本帧期望接收的总长度，用于脏缓存检测和收齐判断 */
}ATFTPPrivate_Struct;
...
static uint8_t ATFTP_FailHandle(uint8_t socketIndex, void * socketPara, uint8_t atTaskID)
{
    ...
    else if (atTaskID == eATFTPCmd_UFSClose)
    {
...
static uint16_t ATFTP_PackUfsClose(uint8_t socketIndex, void * socketPara, uint8_t *pData, uint16_t nATLen)
{
    ...
    /* 无条件关闭文件句柄时，可能存在当前还没有获取到句柄，但是文件确实没有关闭，实测文件句柄大多时为1*/
    if (pPrivate->fileHandle == 0)
    {
        pPrivate->fileHandle = 1;
    }
...
static uint8_t ATFTP_RecvUFSRead(uint8_t socketIndex, void * socketPara, uint8_t *pData, uint16_t dataLen, uint16_t *pDealLen)
{
    ...
    if (pTemp != NULL)
    {
        ...
        if (pTemp != NULL)
        {
            ...
            {
                pTemp += strlen("\r\n");
                payloadLen = dataLen - (uint16_t)((uint32_t)pTemp - (uint32_t)pData);

                if (payloadLen >= readLen)
                {
                    *pDealLen = (uint16_t)((uint32_t)pTemp - (uint32_t)pData) + readLen;

                    if (TRUE == SSUcm_FileDataHandle(pTemp, readLen))
                    {/* 注意: CloseSocket会删除所有发送cmd, 并添加UFSClose,  FTPClose, 如果不返回FALSE, 
                        在CddDrvEG800AK_ATTaskRecvHandle会意外删除UFSClose命令 */
                        ATFTP_CloseSocket(pSocketCtrl);
                        ret = FALSE;
                    }
                    else if (pSocketCtrl->eSocketState == eCddNetMSocketState_Abnormal)
                    {/* SSUcm内部已调CloseSocket，不删命令 */
                        ret = FALSE;
                    }
                    else
                    {/* 正常帧，继续下一帧 */
                ...
                {/* 数据不完整，把已收到的payload存入缓存后再进入透传 */
                    *pDealLen = (uint16_t)((uint32_t)pTemp - (uint32_t)pData) + payloadLen;

                    if (pPrivate->transCacheBuf != NULL)
                    {
                        pPrivate->transCacheLen = 0;
                        pPrivate->transExpectedLen = readLen;
                    }
                    ...
                    {/* 有接收到payload数据 */
                        if (pPrivate->transCacheBuf == NULL)
                        {
                            pPrivate->transCacheBuf = (uint8_t *)myMalloc(SSUCM_CONFIG_SINGLE_FRAME_LEN);
                        }

                        if (pPrivate->transCacheBuf != NULL)
                        {
                            pPrivate->transExpectedLen = readLen;
                            memcpy(pPrivate->transCacheBuf, pTemp, payloadLen);
                            pPrivate->transCacheLen = payloadLen;
...
uint8_t ATFTP_ReavTransparentData(void *modulePara, uint8_t *pData, uint16_t dataLen, uint16_t *pDealLen)
{
    ...
    uint8_t exitTransparent = TRUE;

    /* 异常分支保持原行为：本次数据全部按升级数据消费，避免误送入AT解析。 */
    *pDealLen = dataLen;
    SSUcm_GetReadLenAndOffSet(&readLen, &readOffset);
    ...
    else if (pPrivate->transCacheLen > 0)
    {/* 已有缓存，填充 */
        if (pPrivate->transCacheBuf == NULL || pPrivate->transExpectedLen != readLen)
        {/* 状态不一致(不应发生)，终止OTA */
            SSUcm_SetResult(eSSUcmResult_UnexpectedError);
        }
        else
        {
            /* 只补齐当前升级请求仍缺少的字节，串口数据中多出的部分留给AT层处理。 */
            remainLen = pPrivate->transExpectedLen - pPrivate->transCacheLen;
            copyLen = (dataLen < remainLen) ? dataLen : remainLen;

            memcpy(pPrivate->transCacheBuf + pPrivate->transCacheLen, pData, copyLen);
            pPrivate->transCacheLen += copyLen;
            *pDealLen = copyLen;             /* 多于copyLen的尾部数据未被FTP消费 */

            if (pPrivate->transCacheLen >= pPrivate->transExpectedLen)
            {/* 接收完成，缓存保留供下一帧复用 */
                if (TRUE == SSUcm_FileDataHandle(pPrivate->transCacheBuf, pPrivate->transExpectedLen))
                {
                    ATFTP_CloseSocket(pSocketCtrl);
                }
                else
                ...
                pPrivate->transCacheLen = 0;
                pPrivate->transExpectedLen = 0;
            }
            else
            {
                exitTransparent = FALSE;
            }
        }
    }
    else if (dataLen >= readLen)
    {/* 无缓存，本次完整帧直接处理 */
        /* 前readLen字节属于升级数据，后面的字节可能是同时到达的业务URC。 */
        *pDealLen = readLen;
    ...
    {/* 无缓存，但分帧了 */
        ...
        if (pPrivate->transCacheBuf != NULL)
        {
            ...
            memcpy(pPrivate->transCacheBuf, pData, copyLen);
            pPrivate->transCacheLen = copyLen;
            *pDealLen = copyLen;
    ...
    if (pSocketCtrl->eSocketState == eCddNetMSocketState_Init)
    {
        if (pSocketCtrl->usedFlag == TRUE && CddDrvEG800AK_GetModuleState() == eCddNetMModuleState_Work)
        {
            ...
            if (pFTPPara->eMode == eCddNetMFtpMode_Upload)
            {
                ...
                {
                    CddDrvEG800AK_AddCmd(socketIndex, eATFTPCmd_UFSClose);
...
void ATFTP_CloseSocket(void *socketCtrl)
{
    ...
    if (pPrivate->transCacheBuf != NULL)
    {
        myFree(pPrivate->transCacheBuf);
        pPrivate->transCacheBuf = NULL;
        pPrivate->transCacheLen = 0;
        pPrivate->transExpectedLen = 0;
    }
