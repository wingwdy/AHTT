...
uint8_t SSUcm_FileDataHandle(uint8_t *data, uint32_t dataLen)
{
    ...
    if (headOKflag == TRUE)
    {
        ...
        {
            if (g_stSSUcmCtx.totalFrameCnt > 0)
            {
                g_stSSUcmCtx.progressPercent = (uint8_t)(g_stSSUcmCtx.currentFrameIndex * 100 / g_stSSUcmCtx.totalFrameCnt);
            }

            SSUCM_CFG_DebugPrint("文件下载中[进度：%d%%]: 总包数[%d], 当前包数[%d], 总帧数[%d], 当前帧数[%d]\r\n",
                g_stSSUcmCtx.progressPercent, g_stSSUcmCtx.packCnt, g_stSSUcmCtx.packIndex + 1,
            ...
            if (SSUcm_program_part(FAL_NULL_NAME_UPDATE_PROGRAM, writeAddr, data, dataLen) != TRUE)
            {
    ...
    else if (g_stSSUcmCtx.eUcmWorkState == eSSUcmWorkState_Connecting)
    {
        if (g_stSSUcmCtx.delayFlag == TRUE)
        {
            if (Common_JudgeTimeoutMs(g_stSSUcmCtx.timeoutTick, 2000) == TRUE)
            {
                ...
                if (g_stSSUcmCtx.eChannelType == eSSUcmChannelType_HTTP)
                {
                    /* 初始化分段下载参数(首帧从偏移0开始) */
                    g_stSSUcmCtx.strNetPara.stHttpPara.segPos = 0;
                    g_stSSUcmCtx.strNetPara.stHttpPara.segLen = (uint16_t)g_stSSUcmCtx.readLen;
                    ret = CddNetM_CreatLink(eCddNetMSocketType_HTTP, g_stSSUcmCtx.strNetPara, eCddNetMPlatType_File);
                }
                else
                {
                    ret = CddNetM_CreatLink(eCddNetMSocketType_FTP, g_stSSUcmCtx.strNetPara, eCddNetMPlatType_File);
                }

                if (eGlobalRet_OK != ret)
                {
                    SSUcm_SetResult(eSSUcmResult_UnexpectedError);
                }
            }
...
uint32_t SSUcm_GetFileOffset(void)
{
    return g_stSSUcmCtx.currentFrameIndex * SSUCM_CONFIG_SINGLE_FRAME_LEN;
}
 ...
 ******************************************************************************
*/
uint8_t SSUcm_GetProgress(void)
{
    return g_stSSUcmCtx.progressPercent;
}
