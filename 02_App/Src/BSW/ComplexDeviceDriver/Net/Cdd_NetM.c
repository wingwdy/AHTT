			...
			c_NetMModuleOpsTable[CDD_NETM_CFG_DEV_4G].getModuleState() == eCddNetMModuleState_Work)
		{
			for (index = 0; index < CCDD_NETM_CFG_LINK_COUNT; index++)
			{
				...
				if (pLinkPata->usedFlag == FALSE && pLinkPata->validFlag == TRUE)
				{
					...
					{
						if (TRUE == c_NetMModuleOpsTable[CDD_NETM_CFG_DEV_4G].creatSocket(pLinkPata->eSocketType, &pLinkPata->stSocketPara, 
...
CddNetMHttpPara_Struct* CddNetM_GetHttpPara(CddNetMPlatType_Enum ePlatType)
{
    uint8_t curNetDev = g_stCddNetMCtx.curNetDev;
    uint8_t index = 0;
    CddNetMHttpPara_Struct *ret = NULL;

    for (index = 0; index < CCDD_NETM_CFG_LINK_COUNT; index++)
    {
        CddNetMLinkPara_Struct *pLink = &g_stCddNetMCtx.stLinkPara[curNetDev][index];
        if (pLink->validFlag == TRUE && pLink->ePlatType == ePlatType)
        {
            ret = &pLink->stSocketPara.stHttpPara;
            break;
        }
    }
