...
static uint16_t ATMQTT_PackPublish(uint8_t socketIndex, void * socketPara, uint8_t *pData, uint16_t nATLen)
{
    ...
    if (eGlobalRet_OK == FrameQueue_GetLastTxFrameDataLen(pSocketPara->frameQueueChannelID, &nDataLen, topic, NULL, &qos))
    {
        nATLen = Common_ReplaceNum(pData, nATLen, "[ID]", socketIndex, socketIndex);

        if (qos == 0)
        {
            nATLen = Common_ReplaceNum(pData, nATLen, "[MID]", 0, 0);
        }
        else
        {
            nATLen = Common_ReplaceNum(pData, nATLen, "[MID]", 1, 1);
        }
        ...
        nATLen = Common_ReplaceNum(pData, nATLen, "[QOS]", qos, qos);
    }
    else
    {
        nATLen = 0;
...
static void ATMQTT_PeriodDetectWriteData(CddDrvEG800AKSocketCtrl_Struct *pSocketCtrl)
{
    ...
    uint16_t dataLen = 0;
     ...
     if (eGlobalRet_OK == FrameQueue_GetLastTxFrameDataLen(pSocketPara->stMqttPara.frameQueueChannelID, &dataLen, NULL, NULL, NULL))
     {
        if (dataLen > 0)
        {
...
uint32_t ATMQTT_UrcQMTRecv(uint8_t *pData, void * modulePara, uint16_t dataLen) 
{
    ...
    while (remainingLen > 0)
    {
        ...
        if (pTemp != NULL)
        {
            ...
            if (4 == sscanf((char *)pTemp, "+QMTRECV: %d,%d,\"%128[^\"]\",%d,%n",&socketIndex, &msgId, topic_buf, &payloadLen, &parsed_chars))
            {
