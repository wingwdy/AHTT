...
{
    volatile uint32_t timeout = W25Q_TIMEOUT_VALUE;
    uint8_t status = 0xFF;
...
static uint8_t W25Q_WriteEnable(void)
{
    ...
    uint8_t statusCmd = W25Q_CMD_READ_STATUS1;
    uint8_t status = 0;
    uint8_t ret = FALSE;
    ...
    /* WREN命令不会触发BUSY，必须读取WEL位确认写使能真正生效。 */
    W25Q_CFG_CS_LOW();
    W25Q_CFG_SpiTransmit(&statusCmd, 1);
    W25Q_CFG_SpiReceive(&status, 1);
    W25Q_CFG_CS_HIGH();

    if ((status & W25Q_STATUS_WEL) != 0U)
    {
        ret = TRUE;
    }
...
GlobalRet_Enum W25Q_Read(uint32_t srcAddr, uint8_t* targetAddr, uint32_t len)
{
    ...
    PARA_ASSERT_RET(srcAddr < W25Q_CFG_TOTAL_SIZE && len <= (W25Q_CFG_TOTAL_SIZE - srcAddr), eGlobalRet_ParaInvalid);
    ...
    /* 防止前一次写入或擦除等待超时后，Flash仍处于BUSY状态时继续发送读取命令。 */
    if (W25Q_WaitForIdle() != TRUE)
    {
        eRet = eGlobalRet_Error;
    }
    else
    {
        ...
        W25Q_CFG_SpiReceive(targetAddr, (uint16_t)len);
        W25Q_CFG_CS_HIGH();
    }
...
GlobalRet_Enum W25Q_Write(uint32_t targetAddr, const uint8_t *srcAddr, uint32_t len)
{
    ...
    PARA_ASSERT_RET(targetAddr < W25Q_CFG_TOTAL_SIZE && len <= (W25Q_CFG_TOTAL_SIZE - targetAddr), eGlobalRet_ParaInvalid);
    PARA_ASSERT_RET(g_stW25qxx.initFlag == TRUE, eGlobalRet_NotInit);
    PARA_ASSERT_RET(g_stW25qxx.optState == W25Q_OPT_IDLE, eGlobalRet_DeviceBusy);
