...
GlobalRet_Enum KVDBAdapt_Write(uint16_t blockID, uint8_t *pInBuf, uint16_t dataLen)
{
	KVDBAdaptConfig_Struct *pCfg = &g_stKVDBAdaptConfig;
	char kvName[32] = { 0 };
	struct fdb_blob writeBlob;
	fdb_err_t result = FDB_NO_ERR;
	GlobalRet_Enum eRet = eGlobalRet_Error;

	PARA_ASSERT_RET(blockID < eKVDBAdaptChannel_Count && pInBuf != NULL && dataLen != 0, eGlobalRet_ParaInvalid);

	KVDBAdapt_GeneralKvNameByBlockID(blockID, kvName, sizeof(kvName));
	result = fdb_kv_set_blob(&pCfg->kvdb, kvName, fdb_blob_make(&writeBlob, pInBuf, dataLen));
	eRet = (result == FDB_NO_ERR) ? eGlobalRet_OK : eGlobalRet_Error;

	if (eRet != eGlobalRet_OK)
	{
		FDB_PRINT("[KV] Key '%s' write failed, result:%d, len:%u\r\n", kvName, result, dataLen);
	}
