...
static GlobalRet_Enum FrameQueue_PushMQTT(FrameQueueCtrlDCB_Struct *pDCB, char *pTopic, uint16_t topicLen, uint8_t *pSrcData, uint16_t dataLen, uint8_t direction, uint8_t qos);
...
static GlobalRet_Enum FrameQueue_GetLastFrameDataLen(uint8_t channelID, uint16_t *pDataLen, char *pTopic, uint16_t *pTopicLen, uint8_t direction, uint8_t *pQos);
static GlobalRet_Enum FrameQueue_Push(FrameQueueCtrlDCB_Struct *pDCB, char *pTopic, uint16_t topicLen, uint8_t *pSrcData, uint16_t dataSize, uint8_t direction, uint8_t qos);
...
static GlobalRet_Enum FrameQueue_Push(FrameQueueCtrlDCB_Struct *pDCB, char *pTopic, uint16_t topicLen, uint8_t *pSrcData, uint16_t dataSize, uint8_t direction, uint8_t qos)
{
    ...
    if (pDCB->frameType == eFrameQueueType_MQTT)
    {
        ...
        {
            eRet = FrameQueue_PushMQTT(pDCB, pTopic, topicLen, pSrcData, dataSize, direction, qos);
        }
    }
    else if (pDCB->frameType == eFrameQueueType_TCP)
    {
...
GlobalRet_Enum FrameQueue_PushTx(uint8_t channelID, char *pTopic, uint16_t topicLen, uint8_t *pSrcData, uint16_t dataSize, uint8_t qos)
{ 
    GlobalRet_Enum eRet = eGlobalRet_OK;
    FrameQueueCtrlDCB_Struct *pDCB = &g_stFrameQueueCtrlDCB[channelID];
    PARA_ASSERT_RET(channelID < FRAME_QUEUE_CHANNEL_COUNT, eGlobalRet_ParaInvalid);
    ...
    return FrameQueue_Push(pDCB, pTopic, topicLen, pSrcData, dataSize, FRAME_QUEUE_DIRECTION_TX, qos);
}
...
{ 
    ...
    return FrameQueue_Push(pDCB, pTopic, topicLen, pSrcData, dataSize, FRAME_QUEUE_DIRECTION_RX, 0);
}
...
GlobalRet_Enum FrameQueue_GetLastTxFrameDataLen(uint8_t channelID, uint16_t *pDataLen, char *pTopic, uint16_t *pTopicLen, uint8_t *pQos)
{
    return FrameQueue_GetLastFrameDataLen(channelID, pDataLen, pTopic, pTopicLen, FRAME_QUEUE_DIRECTION_TX, pQos);
}

GlobalRet_Enum FrameQueue_GetLastRxFrameDataLen(uint8_t channelID, uint16_t *pDataLen, char *pTopic, uint16_t *pTopicLen)
{
    return FrameQueue_GetLastFrameDataLen(channelID, pDataLen, pTopic, pTopicLen, FRAME_QUEUE_DIRECTION_RX, NULL);
}
