...
static void IotYKC16_CycleReportRealData(void)
{
    ...
    for (port = 0; port < SYSCFG_CFG_GUN_NUM; port++)
    {
        ...
        if (pIotYKC16Ctx->errVersion[port] != AswErrHandle_GetErrStatusVersion(port))
        {
            ...
            {
                IotTT24_SetRealDataErrBit(port, curErrInfo);
            }
            
            else 
            {
                IotYKC16_SetRealDataErrBit(port, curErrInfo);
            }
        ...
        if (recordSendFlag == FALSE)
        {
                ...
                sizeof(MSNvmOrderInfo_Struct), &pIotYKC16Ctx->time))
            {
                ...
                if (port >= SYSCFG_CFG_GUN_NUM || 
                    (pIotYKC16Ctx->stOrderInfo.protocolType != eAswPlatType_TT24 && pIotYKC16Ctx->stOrderInfo.protocolType != eAswPlatType_YKC16) ||
...
void IotYKC16_TransformBillMode(uint8_t port, AswMonitorBillMode_Struct *pStandardBillMode)
{
    ...
    if (pStandardBillMode != NULL)
    {
        ...
        /* 计费模型有效性校验: billModeID为0表示未被平台写入过计费 */
        if (Common_TwoUint8ToUint16(pYKC16BillMode->billModeID) != 0)
        {
...
uint8_t IotYKC16_FillLinkPara(CddNetMSocketPara_Union *pLinkPara)
{
    MSNvmPlatParam_Struct *pParam = AswPlatM_GetPlatParamPtr();
...
uint8_t IotYKC16_SwipCardCharge(uint8_t port, uint8_t *pCardID)
{
    uint8_t ret = FALSE;
    ...
    if (pIotYKC16Ctx->loginSucc == TRUE)
    {
        if (port < SYSCFG_CFG_GUN_NUM)
        {
            ...
            {
                IOTYKC16_CFG_InfoPrint("[枪：%d]刷卡成功，但是已经有卡在申请启动充电，本次刷卡作废!\r\n", port);
            }
        }
    }
    else
    {
        IOTYKC16_CFG_InfoPrint("[枪：%d]刷卡成功，设备离线，拒绝充电!!\r\n", port);
    }
