...
static void IotOM_CycleDetectUnreportedUcmResult(void)
{
    ...
    if (pIotOMCtx->stProtoData[0].recvUpdateFlag == TRUE)
    {
        ...
        if (UcmResult != eSSUcmResult_None )
        {
            switch (UcmResult)
            {
                case eSSUcmResult_Succ:
                    pIotOMCtx->stProtoData[0].setUpdateResult = eIotOMUpdateResult_Success;
                    break;
                case eSSUcmResult_HeadErr:
                    pIotOMCtx->stProtoData[0].setUpdateResult = eIotOMUpdateResult_FWDevNotMatch;
                    break;
                case eSSUcmResult_GetFileErr:
                case eSSUcmResult_Timeout:
                case eSSUcmResult_DataRecvInterrupt:
                    pIotOMCtx->stProtoData[0].setUpdateResult = eIotOMUpdateResult_DownLoadTimeOut;
                    break;
                case eSSUcmResult_ModuleNoEnoughSpace:
                case eSSUcmResult_UnexpectedError:
                default:
                    pIotOMCtx->stProtoData[0].setUpdateResult = eIotOMUpdateResult_Failed;
                    break;
            }

            pIotOMCtx->stProtoData[0].lastUcmResult = UcmResult;
