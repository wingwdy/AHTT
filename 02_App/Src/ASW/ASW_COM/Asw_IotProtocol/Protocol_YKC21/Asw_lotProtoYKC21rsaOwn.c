...
int encrypt_and_decrypt_data(unsigned char *rsa_key, unsigned char *output, unsigned char *random_key)
{
    ...
    if (ret != 0)
    {
        IOTYKC21_CFG_InfoPrint("Failed to decode Base64: -0x%04X\n", -ret);
        goto exit;
    }
    ...
    if (ret != 0)
    {
        IOTYKC21_CFG_InfoPrint("Failed to parse public key: -0x%04X\n", -ret);
        goto exit;
    }
    ...
    if (ret != 0)
    {
        IOTYKC21_CFG_InfoPrint("Failed to seed the random generator: -0x%04X\n", -ret);
        goto exit;
    }
    ...
    if (ret != 0)
    {
        IOTYKC21_CFG_InfoPrint("Failed to generate the random key: -0x%04X\n", -ret);
        goto exit;
    }
    ...
    if (ret != 0)
    {
        IOTYKC21_CFG_InfoPrint("Encryption failed: -0x%04X\n", -ret);
        goto exit;
    }
    ...
    if (ret != 0)
    {
        IOTYKC21_CFG_InfoPrint("Base64 encoding fails: -0x%04X\n", -ret);
        goto exit;
    }
    // 添加字符串终止符
    if (base64_len >= sizeof(base64_data))
    {
        IOTYKC21_CFG_InfoPrint("Error :Base64 buffer overflow\n");
        goto exit;
    }
...
int encrypt_and_decrypt_data00(unsigned char *rsa_key, unsigned char *output, unsigned char *random_key)
{
   ...
   memcpy(random_key, sym_key, 16);
    ...
    if (ret != 0)
        IOTYKC21_CFG_InfoPrint("mbedtls_ctr_drbg_seed: 0x%08X\n", ret);

    // 2. 加载已知的128字节RSA-512公钥
    IOTYKC21_CFG_InfoPrint("Loading known 128-byte RSA-%d public key...\n", RSA_KEY_BITS);
    ret = load_rsa_public_key(&rsa, rsa_key);
    if (ret != 0)
        IOTYKC21_CFG_InfoPrint("load_rsa_public_key: 0x%08X\n", ret);

    // 3. 用RSA公钥加密16字节对称密钥
    IOTYKC21_CFG_InfoPrint("Encrypting known 16-byte symmetric key...\n");
    ret = mbedtls_rsa_pkcs1_encrypt(&rsa, mbedtls_ctr_drbg_random, &ctr_drbg,
                                    MBEDTLS_RSA_PUBLIC, SYM_KEY_LEN, sym_key,
                                    rsa_encrypted);
    if (ret != 0)
        IOTYKC21_CFG_InfoPrint("mbedtls_rsa_pkcs1_encrypt: 0x%08X\n", ret);

    // 4. 封装为88字节格式
    IOTYKC21_CFG_InfoPrint("Packaging to %d bytes...\n", ENCRYPTED_LEN);
