...
void AswPlatM_PrintAllConfigInfo(void)
{
    ...
    ASWPLATM_CFG_DebugPrint("运营平台环境：%s\r\n", (pParam->envFlag == 0) ? "生产环境" : "测试环境");
...
uint8_t AswPlatM_SetPileDn(char *pPileDn, uint8_t len)
{
    ...
    if (len < MSNVM_PILE_DN_LEN)
    {
        if (len != strlen(g_stAswPlatMCtx.stPlatParam.platPileDn) || memcmp(pPileDn, g_stAswPlatMCtx.stPlatParam.platPileDn, len) != 0)
        {
            ...
            /* 首次设置平台编码时同步固定编码，并通过一次NVM写入保证两个字段的一致性。 */
            if (strlen(g_stAswPlatMCtx.stPlatParam.fixPileDn) == 0)
            {
                ASWPLATM_CFG_DebugPrint("桩固定编码同步变化：[\"%s\"]-->[\"%s\"]\r\n", g_stAswPlatMCtx.stPlatParam.fixPileDn, pPileDn);
                memcpy(g_stAswPlatMCtx.stPlatParam.fixPileDn, pPileDn, len);
            }
        }

        ret = (MSNvm_WriteParaBlock(eMSNvmBlockID_PlatParam, (uint8_t *)&g_stAswPlatMCtx.stPlatParam, sizeof(MSNvmPlatParam_Struct)) == eGlobalRet_OK) ? TRUE : FALSE;
    }
...
uint8_t AswPlatM_SetPlatType(char *platName, uint8_t platNameLen)
{
    ...
    for (index = 0; index < eAswPlatType_Count; index++)
    {
        ...
        if (memcmp(platName, pProtocolDescriptor->pName, platNameLen) == 0)
        {
            ...
            /* 只有平台类型真实变化时才恢复关联参数，重复设置仅交给NVM处理可能存在的失败重试。 */
            if (typeChanged == TRUE && ret == TRUE)
            {
                ret = (MSNvm_SetDefaultParaBlock(eMSNvmBlockID_Gun0Qrcode) == eGlobalRet_OK) ? TRUE : FALSE;
                ret = (ret == TRUE && MSNvm_SetDefaultParaBlock(eMSNvmBlockID_Gun0OrderInfo) == eGlobalRet_OK) ? TRUE : FALSE;
                ret = (ret == TRUE && MSNvm_SetDefaultParaBlock(eMSNvmBlockID_PlatPrivateParam) == eGlobalRet_OK) ? TRUE : FALSE;
            }
...
uint8_t AswPlatM_SwipCardCharge(uint8_t port, uint8_t *pCardID)
{
    const AswPlatMProtocolDescriptor_Struct *pProtocolDescriptor = AswPlatM_GetProtocolDescriptor();
    uint8_t ret = FALSE;
    ...
    {
        ret = pProtocolDescriptor->pFuncSwipCardCharge(port, pCardID);
    }
...
uint8_t AswPlatM_SetDevOperator(char *devOperator, uint8_t len)
{
    ...
    if (devOperator != NULL)
    {
        ...
        {
            ret = c_stAswPlatMProtocolDescriptorTable[currentPlatType].pFuncSetDevOperator(devOperator, len);
        }
    }
...
uint8_t AswPlatM_SetEnvFlag(uint8_t envFlag)
{
    MSNvmPlatParam_Struct *pParam = &g_stAswPlatMCtx.stPlatParam;
    uint8_t ret = FALSE;

    if (envFlag == 0 || envFlag == 1)
    {
        if (envFlag != pParam->envFlag)
        {
            ASWPLATM_CFG_DebugPrint("运行环境变化：[%d]-->[%d]\r\n", pParam->envFlag, envFlag);
            pParam->envFlag = envFlag;
        }

        ret = (MSNvm_WriteParaBlock(eMSNvmBlockID_PlatParam, (uint8_t *)pParam, sizeof(MSNvmPlatParam_Struct)) == eGlobalRet_OK) ? TRUE : FALSE;
    }

    return ret;
}

uint8_t AswPlatM_GetEnvFlag(uint8_t *pEnvFlag)
{
    uint8_t ret = FALSE;

    if (pEnvFlag != NULL)
    {
        *pEnvFlag = g_stAswPlatMCtx.stPlatParam.envFlag;
        ret = TRUE;
    }
...
void AswPlatM_InitMemory(void)
{
    ...
    if (pProtocolDescriptor != NULL)
    {
        ...
        {
            createLink = pProtocolDescriptor->pFuncFillLinkPara(&stSocketPara);
        }
