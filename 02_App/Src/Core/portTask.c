...
#define PORTTASK_CFG_InfoPrint(fmt, ...)           DSLOGM_Info(DSLogMModule_System, fmt, ##__VA_ARGS__)
/*******************************************************************************
*    Enum Definition
*******************************************************************************/
...
static portTask_CtrBlk  g_stTaskCtrBlkTable[] =
{
    ...
    {"App20msA",        Task_20msA,          &g_stTaskCtrBlkTable[2],         256,       8 } ,
...
void portTask_ShowStackInfo(void)
{
    ...
    PORTTASK_CFG_InfoPrint("---------------------------------堆栈信息----------------------------------------\r\n");

    PORTTASK_CFG_InfoPrint("freeRTOS 总堆栈空间: %d K 字节, 剩余空间: %d K 字节\r\n", SYSCFG_CFG_OS_HEAP_SIZE / 1024, xPortGetFreeHeapSize() / 1024);
...
void portTask_CreatAllTask(void)
{
	...
	for (index = 0; index < ARRAY_SIZE(g_stTaskCtrBlkTable); index++)
	{
	    ...
	    if(xResult != pdTRUE)
        {
            PORTTASK_CFG_InfoPrint("%s: Task create failed!!\r\n", pTaskCtr->cTaskName);
        }
        else
        {
